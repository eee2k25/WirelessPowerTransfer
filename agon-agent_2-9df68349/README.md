...
2. Run `npm run dev`. This starts the Expo web app in LAN mode and the room relay on port `8787`.
3. Open the printed Expo web URL on your laptop, then open the same URL using your laptop's LAN IP on the friend's laptop.
...
Both laptops must be on the same Wi-Fi network, and Windows Firewall must allow Node.js/ports `8081` and `8787` on the Private network. The donor creates the room; the receiver enters the displayed code.