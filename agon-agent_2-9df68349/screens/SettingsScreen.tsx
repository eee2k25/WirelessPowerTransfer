...
export default function SettingsScreen() {
  ...
  const saveProfile = () => updateSettings({ vehicleId: id.trim() || settings.vehicleId, vehicleName: name.trim() || settings.vehicleName, esp32Device: dev.trim() || settings.esp32Device });
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.pad} keyboardShouldPersistTaps="handled">
          ...
          <Card theme={theme}>
            ...
            <RowSwitch theme={theme} label="Dark mode" value={settings.darkMode} on={v => updateSettings({ darkMode: v })} />
...
function Field({ theme, value, onChange }: { theme: any; value: string; onChange: (s: string) => void }) {
  return (
    <TextInput
      value={value}
      onChangeText={onChange}
      returnKeyType="done"
      style={{ borderWidth: 1, borderColor: theme.border, backgroundColor: theme.bg, color: theme.text, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, fontWeight: '700' }}
    />
  );
}
function RowSwitch({ theme, label, value, on }: { theme: any; label: string; value: boolean; on: (v: boolean) => void }) {
  return (
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8 }}>
      <Text style={{ color: theme.text, fontWeight: '700', flex: 1 }}>{label}</Text>
      <Switch value={value} onValueChange={on} trackColor={{ true: theme.cyan, false: theme.border }} thumbColor="#fff" />
    </View>
  );
}
function NumRow({ theme, label, value, on, step = 1 }: { theme: any; label: string; value: number; on: (n: number) => void; step?: number }) {
  return (
